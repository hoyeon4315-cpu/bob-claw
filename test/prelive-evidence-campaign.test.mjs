...
test("evidence campaign blocks repeated refresh execution when only audit or policy review gates remain", () => {
  const campaign = buildPreliveEvidenceCampaign({
    reviewPackage: {
      packageStatus: "not_ready_for_policy_review",
      readyForPolicyReview: false,
  ...
  assert.deepEqual(campaign.actions[0].blockers, ["audit:LIVE_BLOCKED", "policy_canary_review_not_ready"]);
  assert.equal(campaign.nextAction.code, "execute_refresh_batch");
  assert.equal(campaign.nextAction.status, "blocked");
});

test("evidence campaign switches to awaiting policy review when fork plan is ready for external signer", () => {
  const campaign = buildPreliveEvidenceCampaign({
    reviewPackage: {
      packageStatus: "not_ready_for_policy_review",
      readyForPolicyReview: false,
  ...
  assert.equal(campaign.overallStatus, "awaiting_policy_review");
  ...
  assert.equal(campaign.actions[3].status, "policy_review");
  assert.match(campaign.actions[3].command, /submit:prelive-fork-execution/);
  assert.equal(campaign.actions[4].status, "blocked");
});

test("evidence campaign routes pending output into policy-review resolution before new fork cycles", () => {
  const campaign = buildPreliveEvidenceCampaign({
    reviewPackage: {
      packageStatus: "not_ready_for_policy_review",
      readyForPolicyReview: false,
  ...
  assert.equal(campaign.actions[4].status, "policy_review");
...
test("evidence campaign suppresses route refresh work once a strategy is policy-review ready", () => {
  const campaign = buildPreliveEvidenceCampaign({
    reviewPackage: {
      packageStatus: "ready_for_policy_review",
