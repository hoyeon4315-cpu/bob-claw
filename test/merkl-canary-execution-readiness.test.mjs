...
test("merkl canary execution readiness blocks when the same portfolio position is open", () => {
  const queueItem = {
  ...
  const positionRecords = [
    {
      ...
      status: "open",
      observedAt: "2026-05-13T11:14:51.385Z",
      opportunityId: "yo-base",
      chain: "base",
      bindingKind: "erc4626_vault_supply_withdraw",
      entryTxHash: "0xentry",
  ...
  assert.equal(readiness.status, "open_position_active");
  assert.deepEqual(readiness.reasons, ["open_position_active"]);
  assert.equal(readiness.openPosition?.entryTxHash, "0xentry");
  assert.equal(annotated.queueStatus, "queued_for_tiny_live_canary_preflight");
  assert.equal(annotated.capabilityGaps.includes("open_position_active"), true);
});
