...
test("canary selection gap skips exact gas refresh while measured leader wallet blockers remain", () => {
  const gap = buildCanarySelectionGap({
    routePlan: {
      topCandidates: [
        {
  ...
  assert.equal(gap.reviewPlan.actionCodes.includes("check_wallet_readiness"), true);
  assert.equal(gap.reviewPlan.actionCodes.includes("refresh_exact_gas"), false);
});
