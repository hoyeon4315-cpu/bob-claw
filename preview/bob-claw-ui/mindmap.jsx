...
function ProtocolChip({ strategy, x, y, size, onTap, selected }) {
  return (
    <g transform={`translate(${x}, ${y})`} onClick={(e) => { e.stopPropagation?.(); onTap?.(); }}
      ...
      <rect x={-size*1.05} y={-size*0.65} width={size*2.1} height={size*1.3} rx={size*0.58}
        fill="#FFFFFF" stroke={selected ? '#111113' : '#DADADA'} strokeWidth={selected ? 1 : 0.6}/>
      <foreignObject x={-size*0.95} y={-size*0.5} width={size*1.0} height={size*1.0}>
        <div xmlns="http://www.w3.org/1999/xhtml" style={{ display:'flex', alignItems:'center', justifyContent:'center', width:'100%', height:'100%' }}>
          <ProtocolLogo id={strategy.protocol} size={size*0.92}/>
        </div>
      </foreignObject>
      <text x={size*0.15} y="3.2" fontSize="9.5" fontWeight="600" fill="#1D1D1F" style={{ fontFamily:'-apple-system, system-ui', textTransform:'capitalize' }}>
        {strategy.protocol}
      </text>
      {strategy.loops && (
...
function Mindmap({ motionSpeed = 1.4 }) {
  ...
  const stratPos = selectedStrategy ? protocolBloom[selectedStrategy] : null;
  const zoom = stratPos ? 2.4 : (selectedChain ? 1.9 : 1);
  const focus = stratPos
    ? stratPos
    : (selectedChain ? (ringPos[selectedChain] || {x:0,y:0}) : {x:0, y:0});
  ...
  return (
    <div style={{
      ...
      <svg width="100%" height="100%" viewBox={`0 0 ${VB} ${VB}`} preserveAspectRatio="xMidYMid meet"
        ...
        <g transform={`translate(${tx}, ${ty}) scale(${zoom})`}
          ...
          {!selectedChain && destChains.map((c) => {
            const p = ringPos[c.id];
            const strats = strategiesByChain[c.id] || [];
            if (strats.length === 0) return null;
            return (
              <EndpointProtocols key={'ep-'+c.id}
                chainId={c.id} chainX={p.x} chainY={p.y}
                strategies={strats} chainSize={chainSize}/>
            );
          })}
      ...
      {selectedChain && !selectedStrategy && (
  ...
  return (
    <div style={{
      ...
      <div style={{ marginTop:10, display:'flex', gap:18, flexWrap:'wrap' }}>
        ...
        <Metric label="Per-tx cap" value={`$${totalCap}`}/>
        <Metric label="Earned" value={totalEarned > 0 ? `$${totalEarned.toFixed(2)}` : '—'} accent={totalEarned > 0}/>
      </div>
      <div style={{ marginTop:8, fontSize:10.5, color:'#8A8A8D', letterSpacing:0.3 }}>
        tap a protocol for details · tap background to reset
      </div>
    </div>
  );
}
