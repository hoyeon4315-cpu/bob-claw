...
const LOCAL_FIRST_PROTOCOL_IDS = /* @__PURE__ */ new Set(["euler", "yo", "gmx", "solv"]);
...
function preloadDashboardLogos() {
  ...
  for (const id of PRELOAD_PROTOCOL_IDS) {
    const logoId = normalizeProtocolLogoId(id);
    const sources = LOCAL_FIRST_PROTOCOL_IDS.has(logoId) ? [LOCAL_PROTOCOL(logoId)] : [...protoSources(id, 28).slice(0, 2), LOCAL_PROTOCOL(logoId)];
