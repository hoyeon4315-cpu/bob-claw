...
- Before feature, policy, dashboard, cleanup, commit, or push work, read
  `docs/system-map.md`, `docs/harness-engineering.md`, and `docs/skill-usage-guidelines.md` after this file.
  The latter three are implementation maps only; this `AGENTS.md` remains the operating
...
**Highest-Priority Rule for All Coding Agents and Subagents.** This section and the following "Coding Agent Operating Mode" section constitute the supreme operating law for every invocation of any coding agent (Claude Code, Codex, Copilot, or successor), every subagent (`.claude/agents/*.md`), every skill (`.claude/skills/*/SKILL.md`), and every coordinator or role-agent delegation pattern.

These two sections have explicit, absolute priority over Reporting Style, Workspace Hygiene, every subsequent section of this document, the content of `docs/skill-usage-guidelines.md` and `docs/ai-agent-operations.md` (in any conflict), and every agent/skill definition file. They are inherited by all subagents and skills with **no escape clauses of any kind** ("verification only", "planning", "read-only", "I am the main agent", "user rephrased", "docs update", etc. are invalid).

Subagent and skill delegation exists solely to keep the primary coding session small, focused, and token-efficient while preserving every cap, policy-engine, kill-switch, receipt-proof, and evidence-complete invariant. Every delegated context inherits the full set of boundaries and procedures from `AGENTS.md` with zero exceptions.

**MUST — Subagent Inheritance Prevention (file scope + diagnostic re-execution + Gateway guard + priority enforcement):**

1. Parent (coordinator or main coding session) MUST, before emitting any delegation, (a) run the literal whole-word "Gateway" check on the intended task, and (b) prefix the child prompt with the exact line `Original Task Name: <verbatim copy of the user's full request>` followed immediately by: "Execute the full 5-step Mandatory Verification Procedure defined in the Coding Agent Operating Mode section of AGENTS.md (the BOB Gateway literal-word check is mandatory step 2) before reading any file, calling any tool, or emitting reasoning. Operate exclusively in Execution Mode: integrate all results into the working implementation and continue. Never emit an unprompted Lx-style status report or multi-item checklist."
2. Every skill `SKILL.md` body and every `.claude/agents/*.md` definition MUST contain, immediately after its frontmatter, the verbatim BOB Gateway Protection (Hard Rules, Enforcement Procedure, and refusal block) from `docs/skill-usage-guidelines.md` plus the full 5-step Mandatory Verification Procedure from the Coding Agent Operating Mode section below.
3. File scope MUST be enforced on every activation before any other work: the task (every file, module, or surface) must lie 100% inside the ownership declared in the agent's frontmatter `description` and the Role Agents table in `docs/ai-agent-operations.md`. Any cross-ownership portion or any reference to a Gateway surface causes immediate refusal, citation of the exact boundary, and return of the task to the parent with zero partial analysis or tool use.
4. Diagnostic re-execution is mandatory and non-skippable: the child MUST invoke the precise AGENTS.md Diagnostic Entry Point CLI(s) matching the question type (capital-audit, full-automation-readiness, payback-status, etc.) and, when the task involves symbol relationships, callers, paths, or likely requires ≥3 source files, MUST first run the appropriate `npm run graph:focus -- query|explain|path` (or direct `python3 -m graphify`) call. The _exact raw command output_ (including failures and "데이터 부족" messages) MUST be pasted verbatim; no summarization, no paraphrase, no substitution from cached knowledge. Before proposing any new CLI or module, `ls src/cli | grep <keyword>` (or script equivalent) MUST be run and its output cited.
5. BOB Gateway guard is absolute and non-overrideable: the literal whole-word occurrence of "Gateway" (capital G, bounded by start/end of string, whitespace, or punctuation — equivalent to `\bGateway\b`) in the `Original Task Name:` or full user request forces the child to emit the exact refusal block defined in `docs/skill-usage-guidelines.md` (the "BOB GATEWAY PROTECTION TRIGGERED" block) and terminate immediately with no tool calls, no file reads, and no further reasoning. Only the primary main coding session operating under direct operator instruction may handle tasks containing the literal word.
6. Priority of this section is total: any subagent or skill definition that omits the required embedded blocks, reorders or weakens step 2, adds escape language, or fails to enforce file scope / diagnostic re-execution is non-compliant and must not be invoked until repaired via committed diff.

**Situation-based automatic judgment companion.** The deterministic logic that lets agents decide without extra user prompting which skill or delegation pattern to use (graphify skill first for topology, bob-claw-readiness-safety-verification skill for blocker/readiness questions, bob-claw-coordinator for multi-ownership features, verifier-agent for post-edit hygiene, or direct Execution Mode for isolated single-ownership work) is supplied by the Master Decision Matrix (and Detailed Situation-Based Decision Guide) in `docs/skill-usage-guidelines.md`. All coding agents, subagents, and skills MUST consult that matrix for automatic, situation-driven routing decisions. `docs/skill-usage-guidelines.md` is the designated situation-based implementation companion and automatic judgment map; `AGENTS.md` remains the operating law on any conflict.

## Coding Agent Operating Mode

**Execution Mode is the universal default.** Every coding agent, skill, subagent, and coordinator invocation begins in Execution Mode and must remain in it for the duration of the task: read the required sources via the 5-step procedure, run all mandated diagnostics and graphify calls, then **immediately perform the requested implementation work** (edits, test additions, config diffs, harness updates, verification). Subagent and skill outputs are raw material to be integrated by the parent; the parent applies the results, executes verification, and continues the implementation without pausing the session for summaries or status reports.

**Explicit Override Declaration (highest priority, durable law).** This Coding Agent Operating Mode section (together with the preceding Subagent Usage section) overrides the following sections of this document for every coding agent, subagent, skill, and coordinator:

| Overridden Section                           | Override Rule (applies to all agent and subagent activity)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Reporting Style                              | The short AGENTS termination format (`현재 단계: Lx`, `이번에 한 일`, `왜 아직 그 단계인지`, `다음 체크리스트` — ≤3 executable items) is emitted **only** at the natural completion of the user's originally requested _unit of work_ (or when the operator explicitly requests a status update). It is **never** emitted after receipt of subagent/skill output, after intermediate edits or auto-commits inside a larger unit, or as an unprompted progress report. Subagent results are integrated silently; the parent stays in Execution Mode and continues working. |
| Workspace Hygiene                            | Auto-commit on "의미 있는 실행 단위" (new CLI+test, policy bundle+regression test, ≥3 related files with same purpose) remains permitted when the primary unit criteria are met. However, the post-commit reporting obligation is satisfied exclusively by the Execution Mode rule above. No intermediate Lx report is required or permitted from delegated contexts, partial units, or subagent flows.                                                                                                                                                                   |
| Any later instruction, example, or narrative | This section pair is the highest-priority rule for all coding-agent and subagent behavior. Narrative tone, "helpful" suggestions, or examples appearing later in AGENTS.md or in skill/agent bodies create no exceptions and do not weaken the numbered MUST procedures or the override table.                                                                                                                                                                                                                                                                            |

**5-Step Mandatory Verification Procedure (MUST be executed in strict sequential order with no omissions, no reordering, and no shortcuts on every skill or subagent activation; quote all headers and raw outputs; integrate evidence then continue implementation):**

1. Re-read in full (quote the `updated_at`, `status`, and version headers of each to prove freshness): `AGENTS.md`, `docs/system-map.md`, `docs/harness-engineering.md`, and `docs/skill-usage-guidelines.md` (the entire BOB Gateway Protection section and the Master Decision Matrix).

2. BOB Gateway Protection literal-word check (absolute internal priority — this step cannot be weakened, moved, made conditional, or skipped): Perform a pure deterministic string match (no LLM reasoning or "understanding" step) for the whole word "Gateway" (capital G, word-bounded) against the `Original Task Name:` line supplied by the parent and the complete original user request. On any match, _immediately_ emit exactly the refusal block defined in the BOB Gateway Protection section of `docs/skill-usage-guidelines.md` and halt. Zero tool calls, zero file reads, zero additional reasoning or analysis are permitted after detection.

3. Enforce file scope: Using only the frontmatter `description` of the current skill/agent and the Role Agents table in `docs/ai-agent-operations.md`, confirm that 100% of the task (every file, directory, module, or surface referenced) lies inside the declared ownership. If any portion falls outside ownership or touches a Gateway surface (`src/gateway/**`, Gateway quote/onramp/offramp paths, payback Gateway leg, `src/config/gateway*.mjs`, `src/executor/policy/gateway-availability.mjs`, or any caller), refuse, cite the precise ownership boundary, and return the task to the parent/coordinator with no other action performed.

4. Execute the AGENTS.md Diagnostic Entry Point(s) and required graphify calls (evidence-complete ground truth — no memory substitution): Invoke the exact Diagnostic Entry Point command(s) from the table in this document that match the question type (e.g., `npm run report:capital-audit -- --json` for NAV/gas/slippage/payback accum; `node src/cli/check-full-automation-readiness.mjs --json` for readiness blockers; `npm run report:payback-status -- --json` for payback/carry; `dashboard/public/dashboard-status.json` for deployed truth). When the task involves code topology, callers, dependency paths, or "explain the relationship", first run `npm run graph:focus -- query|explain|path` (or the direct `python3 -m graphify` equivalent on `src/graphify-out/graph.json`) and use its output to bound all subsequent file reads. Before proposing any new CLI, script, or module, run `ls src/cli | grep <keyword>` (or equivalent under `scripts/`) and cite the output. Paste the _exact raw command output_ (full stdout/stderr, including any failure or empty-data messages) verbatim. Never summarize, paraphrase, or fill gaps with prior knowledge.

5. Perform final hygiene verification, then deliver and continue: Run `git diff --stat`, `git diff --name-only`, `rg` caller search for every deleted or renamed symbol, and the narrow targeted test row(s) from the Verification Matrix in `docs/harness-engineering.md` that apply to the change type ("Any source refactor", "Dashboard UI/status", "Git hygiene", etc.). Confirm that no generated artifacts (`dashboard/public/*.json`, `data/`, `logs/`, coverage, cache) were staged unless the explicit task was to publish a refreshed artifact. **Only after steps 1–5 are fully complete and all raw evidence has been integrated** may the deliverable (code edit, test, diff, or compact report) be produced. Immediately after delivery, continue the remaining implementation work of the unit (apply further patches, run next verification, make additional edits) without emitting any status summary, checklist, or Lx report. The short termination format is emitted **only** when the entire user-requested unit of work reaches its natural completion or the operator explicitly requests status.

**Parent / Coordinator / Main Session MUST (Execution Mode continuation discipline):**

- Remain in Execution Mode for the entire duration of the user's requested unit of work.
- Treat every subagent or skill return as raw material only: apply produced patches to the working tree, execute the relevant hygiene and test commands from the harness Verification Matrix, integrate the evidence, and continue the original implementation task until the unit is complete.
- Never pause the primary session to "report what the subagent said", turn delegation output into an Lx status, or emit intermediate checklists.
- Emit the short AGENTS termination format (`현재 단계: Lx`, `이번에 한 일` (actual changes in 1–3 sentences), `왜 아직 그 단계인지` (fact-based blocker), `다음 체크리스트` (≤3 executable next steps)) **only** at natural completion of the primary unit or on explicit operator status request.

**Child Subagent / Skill MUST:**

- Execute the complete 5-step Mandatory Verification Procedure (Gateway check as step 2) before any other reasoning, file read, or tool call.
- Enforce file scope and the Gateway guard with zero tolerance.
- Return only the compact, evidence-first, directly-usable deliverable that lies inside its declared ownership.
- Never generate an Lx-style report or multi-item checklist unless the parent has declared that the full primary unit of work is now complete.

**Evidence-Complete Confidence Standard (applies universally in Execution Mode and all delegated contexts).** The AGENTS.md Diagnostic Entry Points table supplies ground truth for every NAV, capital, payback, gas, slippage, readiness, or blocker question. The exact CLI is executed and its raw `--json` (or file) output is quoted verbatim in the current session; "데이터 부족" is reported exactly when the command returns no data or fails. No cached assumptions, no "아마", no guess-filled gaps. Before any new module or CLI is proposed, the `ls src/cli | grep <keyword>` check is performed and cited. graphify is the mandatory first tool for topology, caller, and path questions. All of the above is non-negotiable operating law.

The universal Execution Mode default, the explicit override table over Reporting Style and Workspace Hygiene, the 5-step procedure with the BOB Gateway literal check fixed as step 2, the mandatory embedding + file-scope + diagnostic-re-execution + Gateway-guard requirements for every subagent and skill definition, and the required consultation of the `docs/skill-usage-guidelines.md` Master Decision Matrix for all automatic judgment decisions together close every identified loophole (subagent inheritance of reporting style, weak override language, narrative escape clauses, ambiguous verification, premature pausing). This pair of sections is now the highest-priority, durable operating law for all coding agents and subagents. Any future modification requires a committed diff that passes the full harness Verification Matrix, updates the companion `docs/skill-usage-guidelines.md` if needed, and receives explicit operator acceptance against the native-BTC payback safety invariants.
