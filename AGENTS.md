...
4. **Capital Manager** — `src/executor/capital/` — maintains per-chain target balances. Two target sources are supported and merge cleanly:
   ...
   - Score-weighted targets from `src/executor/capital/scored-target-balances.mjs`. Inputs: `destination-promotion-gate.json` items (`score`, `chain`, `strategyId`/`familyId`) and `totalCapitalUsd`. Per-strategy USD = `(score / sumScores) * totalCapitalUsd`, clipped by `strategy.caps.perChainUsd[chain]`. Per-chain settlement target is the sum (not max) of clipped per-strategy targets on that chain.
