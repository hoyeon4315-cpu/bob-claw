...
- **Radar cap graduation.** Cap increases are never automatic. The system may surface a `capRaiseCandidate` only after at least two completed live canaries in the same family have positive realized net PnL after all measured costs, receipt-backed entry and exit/unwind, and at least two distinct campaign windows or opportunity ids. BTC-relative underperformance is reported, but it is not a hard blocker by itself when realized net PnL is positive and the payback conversion path is proven. Any cap raise still requires a committed config diff.
...
- Latest Gateway funding-memory for the live signer:
  ...
  - **Master Auto-Plan v2.0 (2026-04-27):** `src/strategy/master-auto-plan.mjs` implements single-concentration strategy for $520 capital: 67% Aerodrome CL ($350) + 24% YO Protocol ($125) + 3% gas reserve ($15.67) + 6% micro-test ($29.33). Auto-scaling when capital exceeds $650. Auto-rotation of micro-test positions monthly. Full automation with 4-hour ticks, 15-min price monitoring, weekly compounding.
  - **Micro-Test Policy Gate (2026-04-27):** `src/executor/policy/opportunity-policy.mjs` now allows high-risk micro-tests up to $30 and 6% of capital. Auto-exit if >50% loss. Enables small-capital validation of new strategies without risking principal.
