...
import {
  ...
  writeStrategyAutomationLaunchAgents,
...
async function main() {
  ...
  const specs = buildStrategyAutomationLaunchAgentSpecs(specOptions);
  ...
  if (action === "write" || action === "install") {
    writeResult = await writeStrategyAutomationLaunchAgents(specOptions);
  }
