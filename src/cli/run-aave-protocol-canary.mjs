...
import {
  ...
  selectAaveQueueItem,
} from "../executor/helpers/aave-protocol-canary.mjs";
...
async function main() {
  ...
  const queueItem = selectAaveQueueItem(queue, {
  ...
  if (!queueItem) {
    throw new Error("No inventory-ready Aave queue item matched the requested filters");
  }
  ...
  const execution = args.execute
    ? await executeAaveProtocolCanaryPlan({
        plan,
        socketPath: args.socketPath,
        timeoutMs: args.timeoutMs,
      })
  ...
  if (args.write) {
    await writeTextIfChanged(join(config.dataDir, "aave-protocol-canary-latest.json"), `${JSON.stringify(serializableReport, null, 2)}\n`);
