...
import {
  ...
  writeLiveAutomationLaunchAgents,
...
async function main() {
  ...
  const specs = buildLiveAutomationLaunchAgentSpecs(specOptions);
  ...
  if (action === "write" || action === "install") {
    writeResult = await writeLiveAutomationLaunchAgents(specOptions);
  }
