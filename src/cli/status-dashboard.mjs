...
async function main() {
  ...
  if (status.strategy?.merklCanaryQueueSummary) {
    console.log(
      `merklCanaryQueue=queued:${status.strategy.merklCanaryQueueSummary.queueCount ?? 0} top:${status.strategy.merklCanaryQueueSummary.topOpportunityId || "none"} topExecutable:${status.strategy.merklCanaryQueueSummary.topExecutableOpportunityId || "none"} action:${status.strategy.merklCanaryQueueSummary.topNextAction || "none"} bindingReady:${status.strategy.merklCanaryQueueSummary.protocolBindingReadyCount ?? 0} inventoryReady:${status.strategy.merklCanaryQueueSummary.inventoryReadyCount ?? status.strategy.merklCanaryQueueSummary.executableNowCount ?? 0} autoEntryReady:${status.strategy.merklCanaryQueueSummary.autoEntryReadyCount ?? status.strategy.merklCanaryQueueSummary.autoExecutableNowCount ?? 0} executableNow:${status.strategy.merklCanaryQueueSummary.executableNowCount ?? 0} stage:${status.strategy.merklCanaryQueueSummary.executableNowStage || "inventory_ready_before_sizing_policy_and_signer"} blocker:${status.strategy.merklCanaryQueueSummary.topBlockingReason || "none"} executorMissing:${status.strategy.merklCanaryQueueSummary.executorMissingCount ?? 0} cooldown:${status.strategy.merklCanaryQueueSummary.cooldownActiveCount ?? 0} routeGaps:${status.strategy.merklCanaryQueueSummary.chainRouteGapCount ?? 0}`,
    );
  }
