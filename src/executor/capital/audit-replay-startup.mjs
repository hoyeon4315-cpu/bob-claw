...
function hasClosedCapitalAuditRecord(record = {}) {
  ...
  if (record.reconciliationStatus && !["reconciled", "failed", "final_failed"].includes(record.reconciliationStatus)) return false;
  return true;
}

export function replayAuditForCapitalGaps({ auditRecords = [], capitalAuditRecords = [] } = {}) {
  const unmatchedByStrategy = new Map();
  const matchedHashes = new Set(capitalAuditRecords.filter(hasClosedCapitalAuditRecord).map((r) => r.intentHash).filter(Boolean));
  ...
  for (const record of auditRecords) {
    ...
    const gapKey = record.intentHash || `missing_intent_hash:${record.strategyId || "unknown"}:${record.lifecycle?.txHash || record.broadcast?.txHash || record.txHash || recordTimestamp(record) || "unknown"}`;
