...
function filterIgnoredTreasurySnapshots(treasurySnapshots = [], ignoredTreasuryAddresses = []) {
  const ignored = new Set((ignoredTreasuryAddresses || []).map(normalizeAddress).filter(Boolean));
  if (!ignored.size) return treasurySnapshots || [];
  return (treasurySnapshots || []).filter((snapshot) => !ignored.has(normalizeAddress(snapshot?.address)));
}
...
export function buildCapitalAuditReport({
  ...
  ignoredTreasuryAddresses = [],
} = {}) {
  const broadcasts = latestBroadcastSignerRecords(signerAuditRecords);
  const usableTreasurySnapshots = filterIgnoredTreasurySnapshots(treasurySnapshots, ignoredTreasuryAddresses);
  const scope = buildCapitalAuditScope({
    signerAuditRecords,
    treasurySnapshots: usableTreasurySnapshots,
    gatewayBtcOfframpExecutions,
    gatewayBtcOnrampExecutions,
    approvedOperatorBtcAddresses,
  });
  ...
  const enrichedTreasurySnapshots = injectProtocolPositionsIntoLatestSnapshot(
    usableTreasurySnapshots,
    protocolPositionMarks,
  );
