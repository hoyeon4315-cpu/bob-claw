...
Updated: 2026-04-15T07:03:40.573Z
...
- Proxy lane: proxy=`wbtc` action=`expand_amount_ladder` quota=30
...
- Validation: status=`blocked` nextStage=`shadow_replay` nextAction=`hold_dex_quote`
- Connected refresh: status=`blocked_nonrefreshable_input` required=0 next=`hold_dex_quote`
- Refresh runner state: runs=7 preview=1 latest=`succeeded` remaining=0
- Connected pre-live pass: `npm run run:current-route-prelive-pass -- --execute`
- Current-route pass report: `npm run report:current-route-prelive-pass -- --write`
- Exact-route fork split: status=`missing_exact_route_plan` technical=`missing_plan` economic=`blocked_no_net_edge`
- Judgment review: status=`guarded_blocked` issues=2 high=1
...
- Strategy lane: `btc_proxy_spreads` status=`thin_coverage` floor=$7.58
...
- Runbook next action: `hold_dex_quote`
...
- Validation: status=`blocked` readiness=0% blockers=11 warnings=4
- Validation next step: stage=`shadow_replay` action=`hold_dex_quote`
- Validation headline: topStrategy=`stablecoin_entry_exit_loops` topPivot=`gateway_base_btc_yield`

## Connected Refresh Package

- Connected refresh: status=`blocked_nonrefreshable_input` route=`avalanche->bera wBTC.OFT->wBTC.OFT` amount=`10000` required=0
- Refresh next step: action=`hold_dex_quote`
...
- Refresh execution: runs=7 preview=1 success=7 partial=0 failed=0 latest=`succeeded`
- Refresh execution next: action=`hold_dex_quote`
- Current-route pre-live pass: runs=7 preview=1 readyForSigner=0 blocked=7 partial=0 failed=0 latest=`blocked_nonrefreshable_input`
...
- Fork next step: action=`hold_dex_quote`

## Operational Judgment Review

- Operational judgment: status=`guarded_blocked` issues=2 high=1 medium=1
- Judgment next step: action=`hold_dex_quote`
...
- Remaining steps: resolve blocked inputs (DEX quote) · clear objective blocker (reject_no_net_edge) · advance canary beyond BLOCKED_NO_VIABLE_PREP_ROUTE
...
- Net edge now: $-0.5605
- Objective score blocker: reject_no_net_edge (net edge $-0.5605)
...
- active_canary route=`avalanche->bera wBTC.OFT->wBTC.OFT` amount=`10000` txReady=true viableForPrep=true net=$-0.5605 prepFunding=$0.0000 blockers=none priority=high_quote_latency evidence=shadow:12 quotes:7/7 success:100.0% p95:3598ms fee:$0.5236 reasons:reject_effective_system_pnl:12,reject_treasury_execution_refill_cost:12,insufficient_data:6,reject_no_net_edge:6,exact_src_execution_gas_not_estimated:5
- tx_ready_shadow route=`avalanche->bob wBTC.OFT->wBTC.OFT` amount=`10000` txReady=true viableForPrep=false net=$-0.7936 prepFunding=$0.0000 blockers=score:stale_src_gas_snapshot priority=evidence_accumulating evidence=shadow:12 quotes:3/3 success:100.0% p95:1579ms fee:$0.7564 reasons:exact_src_execution_gas_not_estimated:12,insufficient_data:12,reject_effective_system_pnl:12,reject_treasury_execution_refill_cost:12,stale_src_gas_snapshot:11
- tx_ready_shadow route=`bob->avalanche wBTC.OFT->wBTC.OFT` amount=`10000` txReady=true viableForPrep=false net=$-0.8500 prepFunding=$0.0000 blockers=score:stale_src_gas_snapshot priority=evidence_accumulating evidence=shadow:12 quotes:3/3 success:100.0% p95:1018ms fee:$0.8128 reasons:exact_src_execution_gas_not_estimated:12,insufficient_data:12,reject_effective_system_pnl:12,reject_treasury_execution_refill_cost:12,stale_src_gas_snapshot:9
- tx_ready_shadow route=`bob->base wBTC.OFT->wBTC.OFT` amount=`10000` txReady=true viableForPrep=false net=$-0.8637 prepFunding=$0.0000 blockers=score:stale_src_gas_snapshot priority=evidence_accumulating evidence=shadow:42 quotes:22/22 success:100.0% p95:622ms fee:$0.8265 reasons:reject_effective_system_pnl:40,reject_treasury_execution_refill_cost:40,reject_no_net_edge:22,insufficient_data:20,stale_dex_output_quote:13
- tx_ready_shadow route=`bob->sonic wBTC.OFT->wBTC.OFT` amount=`10000` txReady=true viableForPrep=false net=$-0.8513 prepFunding=$0.0000 blockers=score:stale_src_gas_snapshot priority=evidence_accumulating evidence=shadow:12 quotes:3/3 success:100.0% p95:501ms fee:$0.8142 reasons:exact_src_execution_gas_not_estimated:12,insufficient_data:12,reject_effective_system_pnl:12,reject_treasury_execution_refill_cost:12,stale_src_gas_snapshot:8
...
- tx_ready_shadow route=`bob->base wBTC.OFT->wBTC.OFT` next=refresh_exact_gas reason=stale_src_gas_snapshot command=`npm run estimate:gateway-gas -- --route-key="bob:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->base:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c" --amount="10000" --from="0x96262be63aa687563789225c2fe898c27a3b0ae4"`
...
- rank=6 priority=80 scope=tx_ready_shadow next=refresh_exact_gas reason=stale_src_gas_snapshot route=`bob->base wBTC.OFT->wBTC.OFT` amount=`10000` command=`npm run estimate:gateway-gas -- --route-key="bob:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->base:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c" --amount="10000" --from="0x96262be63aa687563789225c2fe898c27a3b0ae4"`
...
- Pivot budget: current=$300.00 note=USD 300 is the configured safety budget and loss cap, not a claim that every viable strategy should naturally fit inside USD 300.
...
- Alternate pivot: `btc_proxy_spreads` status=`blocked_current_surface` label=`BTC proxy spread arbitrage` observedFloor=$7.58
...
- Proxy coverage: overfit=`high_overfit_risk` plans=1 actionable=1 quota=30
- Next proxy target: proxy=`wbtc` action=`expand_amount_ladder` reason=`partial_amount_match` priority=`high` quota=30
...
- Summary: status=`not_ready_for_manual_review` review=`NOT_READY_FOR_MANUAL_CANARY_REVIEW` live=`LIVE_EXECUTION_BLOCKED` stage=`shadow_replay` blockers=manual_review_stage_not_ready,shadow_replay_not_ready,mechanical_simulation_not_ready,fork_execution_not_ready,reject_no_net_edge,blocked_dex_quote
- Tiny canary admission: decision=`NO_GO` status=`blocked` blockers=manual_review_stage_not_ready,shadow_replay_not_ready,mechanical_simulation_not_ready,fork_execution_not_ready,reject_no_net_edge,blocked_dex_quote next=`clear_admission_blockers`
- Admission remediation: status=`ready` ready=1 manual=0 blocked=1
- Admission remediation runner: `npm run run:admission-remediation -- --execute --limit=1`
- Admission next action: `execute_refresh_batch` status=`ready` command=`npm run run:shadow-refresh-batch -- --execute --limit=1`
- Admission remediation item: rank=1 status=`blocked` code=`hold_dexQuote` reason=blocked_dex_quote
...
- Manual review candidate: route=`avalanche->bera wBTC.OFT->wBTC.OFT` amount=`10000` readiness=`reject_no_net_edge` net=$-0.5605 prepFunding=$0.0000 txReady=true viableForPrep=true
- Candidate inputs: quote fresh (0.2m) · exactGas fresh (0.2m) · srcGas fresh (0.2m) · dex blocked · btcFee not_needed · market fresh (0.1m)
...
- Candidate evidence: shadow=12 quotes=7/7 success=100.0% p95=3598ms routeFailure=0.0%
...
- Review checklist: completed=top canary route selected · tx payload captured · wallet readiness cleared · exact gas captured remaining=resolve blocked inputs (DEX quote) · clear objective blocker (reject_no_net_edge) · advance canary beyond BLOCKED_NO_VIABLE_PREP_ROUTE
...
- Overfit audit: LIVE_BLOCKED · sample=shadow_observations · horizon=91.0h · buckets=23
- Overfit blockers: shadow time window, time bucket diversity
- Overfit warnings: legacy records, gas snapshot failures
- Overfit runway: 77.0h remaining to 168h · 1 hourly buckets remaining to 24
- Overfit time ETA: shadow window 2026-04-18T12:00:56.658Z · bucket diversity 2026-04-15T08:00:00.000Z · earliest time-gate pass 2026-04-18T12:00:56.658Z
...
- Current canary route: reject_no_net_edge net=$-0.5605
- Closest route to policy: none observed
- Best stablecoin route tested: `base:0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913->bitcoin:0x0000000000000000000000000000000000000000` amount=`4022463` readiness=`insufficient_data` net=$-0.9099
...
- Best ETH-family research route: `bitcoin:0x0000000000000000000000000000000000000000->base:0x0000000000000000000000000000000000000000` class=`unknown` readiness=`observe_only_slow_settlement` net=$-0.5602
...
- Best DEX focus route now: `base:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->ethereum:0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599` class=`loop_observable` gatewayQuotes=13 entryQuotes=6 exitQuotes=9 bestExec=$-1.3632
- DEX environment drift: monitored=36 staleLegs=15 unstableLegs=16 thinLiquidityLegs=3 singleSampleLegs=0
- Top DEX environment risk: `base:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->soneium:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c` amount=`10000` class=`refresh_needed` staleLegs=2 unstableLegs=0 thinLiquidityLegs=0 singleSampleLegs=0
- Best research route now: `bitcoin:0x0000000000000000000000000000000000000000->bob:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c` class=`no_edge` profitableLevels=0/6 bestNet=$-0.2016
- Measured DEX+Gateway coverage: bothDexSupported=71 executable=20 measuredNet=3 exact=7 profitable=0
- Closest route to policy gate: none observed
- Best persistence route now: `base:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->ethereum:0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599` class=`durable_no_edge_route` measuredLevels=3 minGap=n/a bestNet=$-1.4463
- Closest measured DEX+Gateway loop: `base:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->ethereum:0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599` netEdge=$-1.4463 amountGap=2.04% blockers=entry_amount_mismatch,gateway_stale_src_gas_snapshot,gateway_exact_src_execution_gas_not_estimated,gateway_stale_dex_output_quote,gateway_observe_only_ethereum_l1_phase_disabled,non_positive_loop_net_edge
- Best stablecoin-related route now: `base:0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913->bitcoin:0x0000000000000000000000000000000000000000` amount=`4022463` readiness=`insufficient_data` netEdge=$-0.9099
...
- Chain price coverage: observed 3, stale 0, missing 6
...
- Route input freshness: quote fresh (0.2m) · exactGas fresh (0.2m) · srcGas fresh (0.2m) · dex blocked · btcFee not_needed · market fresh (0.1m)
- Route input blockers: reject_no_net_edge
- Canary input watcher: skip avalanche:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->bera:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c amount=10000 (current canary route inputs are fresh)
...
- Blocked-score watcher: skip avalanche:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c->bera:0x0555E30da8f98308EdB960aa94C0Db47230d2B9c amount=10000 (no new score inputs arrived)
