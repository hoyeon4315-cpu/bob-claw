...
- `radar.status`
- `radar.headline`
- `radar.stageCounts.observed`
- `radar.stageCounts.hypothesis`
- `radar.stageCounts.portable`
- `radar.stageCounts.executableReview`
- `radar.stageCounts.selfRealized`
- `radar.stageCounts.positiveRealizedPnl`
- `radar.stageCounts.paybackDelivered`
- `radar.pnl.totalNetRealizedPnlUsd`
- `radar.pnl.totalNetRealizedPnlSats`
- `radar.capReview.eligibleCount`
- `radar.capReview.lossLockOn`
- `radar.capReview.topSuggestedNextTinyLivePerTxUsd`
- `radar.topBlocker`
- `radar.guardrails.readOnly`
- `radar.guardrails.noExecution`
- `tradeHistory.count`
...
When no live endpoint is available, the browser reloads `dashboard-status.json` every 10 seconds with `cache: "no-store"` and a timestamp query string. When the local read-only live server is available, the browser may switch to `/api/live-status` polling and `/api/live-events` SSE for faster updates.
