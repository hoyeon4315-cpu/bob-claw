...
**5단계 후보** — score-weighted bootstrap + bidirectional rebalance + 단일 체인/전략 dominance guard + BSC fallback lane까지 코드 완료. 실전 dry-run에서 11체인 분배 실측은 다음 자동 tick에서 확인.
...
- 남은 pre-existing failing tests (10건): 모두 main 기준 동일 — `Gateway official chains exposed/=11` (diversification.mjs 중복 `bnb`/`bsc` alias로 13개), `deploy main` (Cloudflare 환경), `Gas.Zip refill preparation blocks`, `operational address`, `promotion evidence/pr-preview`, `supportedBindingKinds`, `status dashboard` — 본 작업 범위 외.
- 실전 dry-run 11체인 분배는 `data/destination-admission-checklist.json` 등 워크트리 산출물이 없는 상태에선 측정 불가. 운영 환경에서 `npm run executor:bootstrap-from-btc -- --total-capital-usd=...` 실행하면 score-weighted + diversification cap 적용된 per-chain refill plan이 생성됨.
