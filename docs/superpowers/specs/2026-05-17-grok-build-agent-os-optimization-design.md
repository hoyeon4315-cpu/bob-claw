...
7. **Grok 세션 Worktree 규칙 강제**: 기본은 메인 worktree. 별도 worktree 사용 시 main-worktree-applicable raw patch 또는 commit SHA 필수. 증거: pwd / current branch / git diff / changed file content (이 규칙은 AGENTS.md, docs/ai-agent-operations.md, AGENT-SUPREME-LAW.md에 이미 반영되며, Grok Build 관련 운영 문서에도 동일하게 적용).
...
_이 문서는 brainstorming skill 프로세스를 통해 작성되었으며, 사용자 승인 후 `writing-plans` skill로 넘어가 구현 계획을 수립할 예정입니다._
