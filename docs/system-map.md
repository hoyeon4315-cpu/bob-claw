...
| Aggressive Velocity Sleeve Accounting (defi-portfolio-accounting skill) | Thin SKILL.md orchestrator + thick pure `src/ledger/aggressive-sleeve-accounting.mjs` (TDD, 15 pitfalls, lot accounting, IL/velocity). Append-only `data/aggressive-yield/ledger.jsonl` + derived snapshots. Additive payback attribution only; core payback/Gateway untouched. 4 subagents consume pure lib directly for hot decisions. |
...
| `data/aggressive-yield/*.jsonl` + `*.json` | Aggressive Velocity Sleeve append-only ledger, asset tracker, performance, payback attribution (defi-portfolio-accounting skill) | Ignored                       | Operational; sleeve-specific slice of signer-audit + protocol marks. Never commit live numbers. |
