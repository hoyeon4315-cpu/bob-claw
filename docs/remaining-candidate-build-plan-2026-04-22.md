...
4. auto-promotion dev-guard 결과를 dashboard와 session handoff에 반영
...
3. 운영 promotion 단계:
   - legacy receipt-promotion evidence was removed; current commit guard uses auto-promotion preview evidence.
...
1. `live_candidate` 유지
2. auto-promotion dev guard returns an eligible preview
3. lookback window에서:
...
1. auto-promotion preview evidence를 strategy status/report pipeline에 연결
...
Evidence:
- `src/cli/report-strategy-tick-slice.mjs`: auto-promotion preview evidence를 readiness map으로 만들어 `buildStrategyStageSlice`에 전달
...
- `wrapped-btc-loop-base-moonwell` 포함 최소 1개 전략에 대해 auto-promotion preview 결과가 artifact로 노출된다.
...
4. auto-promotion dev guard fast-track threshold 충족 (signer-backed receipts ≥ 2, consecutive success ≥ 1, etc.)
