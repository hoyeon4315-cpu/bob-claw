...
| Any new skill or agent added    | `node --test test/skills-config.test.mjs` (enforces Supreme Law reference + scope/ownership guardrails + 5-step Mandatory Verification Procedure + "Execution Mode" language in all SKILL.md + agent .md, and rejects the removed Gateway-specific refusal rule); `npm run check:skills-config`; update skill-usage-guidelines.md matrix first |
