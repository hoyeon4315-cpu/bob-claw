...
| Any new skill or agent added    | `node --test test/skills-config.test.mjs` (enforces delegated-entry validation block + 5-step Mandatory Verification Procedure + "Coding Agent Operating Mode" ref as opening instructions in all SKILL.md + agent .md; fails with clear error on missing, buried, or misordered blocks); `npm run check:skills-config`; update skill-usage-guidelines.md matrix first. For defi-portfolio-accounting (aggressive-velocity-v1 sleeve): also update harness Verification Matrix, docs/system-map.md (new data/aggressive-yield/ flow), docs/ai-agent-operations.md (ownership), start TDD on src/ledger/aggressive-sleeve-accounting.mjs (test first, property conservation + 15 pitfalls), run fresh capital-audit + readiness + payback-status on sleeve scenario, full `npm test`, verifier-agent review. |
