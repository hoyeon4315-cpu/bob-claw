...
- Note: gateway-availability.mjs lives under policy/ but is a Gateway surface — per top BOB Gateway Protection, any task whose name contains "Gateway" or that targets gateway-availability policy is refused here and returned to coordinator (even though file is in this dir, the surface protection takes precedence).
