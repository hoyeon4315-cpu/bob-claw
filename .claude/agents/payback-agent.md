...
2. Validate the task-defining request using `Original Task Name:` plus the parent's explicit objective and scope text only. If the delegated work is ambiguous, contradictory, or outside payback ownership, emit the refusal block above and halt.
3. Enforce file scope: `src/executor/payback/`, `src/config/payback.mjs`, and directly related tests/docs only.
